...
using System.IO;
using Satis.Importers.Autodesk3ds.Util;
 ...
 */
	public class Autodesk3dsLoader
	{
		public bool ProcessChunk(BinaryReader stream, Action<Discreet3DS.Chunk> callback)
		{
			// ------------------------------------------------------------------------------------------------
			...
			while (true)
			{
				if (stream.BaseStream.GetRemainingSizeToLimit() < sizeof(Discreet3DS.Chunk))
					return false;
...
public bool CanRead(string pFile, IOSystem* pIOHandler, bool checkSig)
{
...
public void GetExtensionList(List<string> extensions)
{
	extensions.Add("3ds");
	extensions.Add("prj");
}
...
private void InternReadFile( string pFile, aiScene* pScene, IOSystem* pIOHandler)
{
...
private void ApplyMasterScale(aiScene* pScene)
{
...
private void ParseMainChunk()
{
	ProcessChunk(chunk =>
	{
		...
		switch (chunk.Flag)
		{
			...
			case Discreet3DS.ChunkType.CHUNK_PRJ:
				bIsPrj = true;
				break;
			case Discreet3DS.ChunkType.CHUNK_MAIN:
				ParseEditorChunk();
				break;
		}
	});
